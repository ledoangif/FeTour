...
import { withTrailingSlash, withLeadingSlash, hasProtocol, joinURL, withoutLeadingSlash } from 'ufo';
...
const google = {
  ...
  async resolveFontFaces(fontFamily, defaults) {
    ...
    return {
      fonts: await cachedData(`google:${fontFamily}-${hash(defaults)}-data.json`, () => getFontDetails$3(fontFamily, defaults), {
        onError(err) {
...
async function initialiseFontMeta$3() {
  fonts$3 = await cachedData("google:meta.json", fetchFontMetadata, {
    onError() {
...
function isGoogleFont(family) {
  return fonts$3.some((font) => font.family === family);
}
const styleMap = {
  italic: "1",
  oblique: "1",
...
async function getFontDetails$3(family, variants) {
  const font = fonts$3.find((font2) => font2.family === family);
...
const bunny = {
  ...
  async resolveFontFaces(fontFamily, defaults) {
    ...
    return {
      fonts: await cachedData(`bunny:${fontFamily}-${hash(defaults)}-data.json`, () => getFontDetails$2(fontFamily, defaults), {
        onError(err) {
...
async function initialiseFontMeta$2() {
  fonts$2 = await cachedData("bunny:meta.json", () => fontAPI$2("/list", { responseType: "json" }), {
    onError() {
      logger.error("Could not download `bunny` font metadata. `@nuxt/fonts` will not be able to inject `@font-face` rules for bunny.");
      return {};
    }
  });
  for (const id in fonts$2) {
    familyMap$1.set(fonts$2[id].familyName, id);
  }
}
...
async function getFontDetails$2(family, variants) {
  const id = familyMap$1.get(family);
  const font = fonts$2[id];
...
const fontshare = {
  ...
  async resolveFontFaces(fontFamily, defaults) {
    ...
    return {
      fonts: await cachedData(`fontshare:${fontFamily}-${hash(defaults)}-data.json`, () => getFontDetails$1(fontFamily, defaults), {
        onError(err) {
...
async function initialiseFontMeta$1() {
  fonts$1 = await cachedData("fontshare:meta.json", async () => {
    const fonts2 = [];
    let offset = 0;
    let chunk;
    do {
...
async function getFontDetails$1(family, variants) {
  const font = fonts$1.find((f) => f.name === family);
  const numbers = [];
...
async function getAdobeFontMeta(id) {
  const { kit } = await fontAPI(`/api/v1/json/kits/${id}/published`, { responseType: "json" });
  return kit;
}
async function initialiseFontMeta(kits) {
  fonts = {
